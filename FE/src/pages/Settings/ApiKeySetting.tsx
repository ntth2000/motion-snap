...
const ApiKeySetting = () => {
  ...
  const generateApiKey = async () => {
    try {
      ...
      messageApi.open({
        content: t('pages.settings.apiKey.message.regenerateSuccess'),
        type: "success",
      });
    } catch (err) {
      messageApi.open({
        content: t('pages.settings.apiKey.message.regenerateFailed'),
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const copyApiKey = async () => {
    ...
    messageApi.open({
      content: t('pages.settings.apiKey.message.copySuccess'),
      type: "success",
    });
  };
  ...
  return (
    <div>
      ...
      <Title level={5}>
        <KeyOutlined />
        <span style={{ marginLeft: 4 }}>{t('pages.settings.apiKeySetting.title')}</span>
      </Title>
      <Divider />
      <div style={{ marginBottom: 24 }}>
        <Text type="secondary" style={{ whiteSpace: 'pre-line' }}>
          {t('pages.settings.apiKeySetting.description')}
        </Text>
      </div>
      {
        apiKey.key ? (
          <Space direction="vertical" size="middle">
            <Text strong>{t('pages.settings.apiKeySetting.subTitle')}</Text>
            ...
            <div>
              ...
              <Space>
                {apiKey.canCopy && <Button
                  ...
                  onClick={copyApiKey}
                >
                  {t('pages.settings.apiKeySetting.copyBtn')}
                </Button>}
                ...
                >
                  {t('pages.settings.apiKeySetting.regenerateBtn')}
                </Button>
              </Space>
            </div>
          </Space>
